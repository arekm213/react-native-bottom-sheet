...
top of the sheet from `event.nativeEvent.position`. The same event also carries
`event.nativeEvent.index`—the fractional detent index in
...
To keep the latest position in a Reanimated shared value, update it from the
callback:
